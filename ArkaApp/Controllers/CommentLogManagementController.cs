...
{
    [Authorize]
    public class CommentLogManagementController : Controller
    {
        Models.Repositories.UnitOfWork _unitOfWork;
        public CommentLogManagementController()
        {
            ViewBag.ActiveMenu = "comments";
            _unitOfWork = new Models.Repositories.UnitOfWork();
        }

        public ActionResult Index(CommentSearchViewModel model)
        {
            const int RecordsPerPage = 10;

            List<Models.EntityModels.CommentLog> CommentLogs = _unitOfWork.Repository<Models.EntityModels.CommentLog>().Get().ToList();

            if (!string.IsNullOrEmpty(model.SearchButton))
            {
                if (model.Id != 0)
                    CommentLogs = CommentLogs.Where(x => x.Post.AccountID == model.Id).ToList();
            }

            var pageIndex = model.Page ?? 1;
            model.SearchResults = CommentLogs.ToPagedList(pageIndex, RecordsPerPage);
        ...
        public ActionResult Detail(int? id)
        {
            ...
            var model = _unitOfWork.Repository<Models.EntityModels.CommentLog>().GetById(id);
        ...
        public ActionResult GetLineChartData(int? id)
        {
            ...
            if (model != null)
            {
                ...
                string query = "SELECT CAST(CommentLogDetail.[Date] AS date) AS Label, MIN(CommentLogDetail.count) - (SELECT CommentLog.PreviousCount FROM CommentLog WHERE CommentLogID = 1) + (MAX(CommentLogDetail.Count) - MIN(CommentLogDetail.count)) AS Value" +
                                " FROM dbo.CommentLogDetail JOIN CommentLog ON CommentLog.CommentLogID = CommentLogDetail.CommentLogID" +
                                " WHERE CommentLogDetail.CommentLogID = " + model.CommentLogID +
                                " GROUP BY CAST(CommentLogDetail.[Date] AS date)";
        ...
        public ActionResult GetLiveChartData(int? id)
        {
            ...
            var details = _unitOfWork.Repository<Models.EntityModels.CommentLogDetail>().Get(x => x.CommentLogID == id).ToList();

            if (details.Count > 1)
                lastCount = details.OrderByDescending(x => x.Date).First().Count;

            //call api
            _unitOfWork.Repository<Models.EntityModels.CommentLogDetail>().Insert(new Models.EntityModels.CommentLogDetail
            {
                Count = lastCount + 500,
                Date = DateTime.Now,
                CommentLogID = id
            ...
            if (model != null)
            {
                ...
                string query = "SELECT CommentLogDetail.[Date] AS Label, CommentLogDetail.Count AS Value" +
                                " FROM dbo.CommentLogDetail JOIN CommentLog ON CommentLog.CommentLogID = CommentLogDetail.CommentLogID" +
                                " WHERE CommentLogDetail.CommentLogID = " + model.CommentLogID;
